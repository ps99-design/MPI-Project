...
def forward_transform(psi, comm):
    ...
    psik = fft.forward(psi, normalize=True)
    return psik

def inverse_transform(psik, comm):	#without "comm" it is also working.
    N = np.array([para.Nx, para.Ny, para.Nz], dtype=int)
    fft = PFFT(comm, N, axes=(0, 1, 2), dtype=np.complex128, grid=(-1,))
    # psik=dealias(psik)
    #N = np.array([para.Nx, para.Ny, para.Nz], dtype=int)
    ...
    psi = fft.backward(psik)	#deleting "psi"
    return psi
...
def get_kx_ky_kz(comm):
	Dx=np.arange(-para.Nx//2+1,para.Nx//2+1)
	kx=2*np.pi*np.roll(Dx,para.Nx//2+1)/para.Lx

	Dy = np.linspace(comm.Get_rank()*para.Ny/comm.Get_size(), ((comm.Get_rank()+1)*para.Ny/comm.Get_size())-1, para.Ny//comm.Get_size() ) - ( para.Ny//2 -1 ) 
	ky = (2*np.pi/para.Ly)*np.roll(Dy, para.Ny//(2*comm.Get_size()) -1)
